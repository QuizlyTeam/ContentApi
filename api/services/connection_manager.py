...
from ..schemas.quizzes import GameJoinModel, GameAnswerModel
...
class ConnectionManager(AsyncNamespace):
    ...
    async def get_questions(self, sid: str, options: dict):
        ...
        options_without_exclude_keys = {k: options[k] for k in set(list(options.keys())) - set(exclude_keys)}
    ...
    async def send_questions(self, sid: str):
        ...
        for index, q in enumerate(questions):
            ...
            await sleep(18) # 15 seconds for answer and 3 seconds for showing results
        await self.send_results(sid)

    async def on_connect(self, sid: str, environ: dict):
        await self.emit('test', self.connections, room=sid)
    ...
    async def on_join(self, sid: str, options):
        ...
        if options['max_players'] == 1:
            ...
            await self.update_session(sid, { 'points': {sid: 0}, 'current_question': 0, 'names': { sid: options['name'] }})
            await self.ready(sid)
    ...
    async def on_answer(self, sid: str, data: dict):
        ...
        correct_answer = session_data['questions'][current_question]['correct_answer']
